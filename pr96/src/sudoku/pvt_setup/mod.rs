...
pub fn pvt_cell_calc_pvs(
    puzzle: &Matrix,
    pvt: &mut Vec<Vec<Option<Vec<u8>>>>,
    &row: &usize,
    &col: &usize,
) {
    ...
    if possible_vals.len() == 0 {
        pvt[row][col] = None;
    }
}

pub fn pvt_cell_check_sqr(
    puzzle: &Matrix,
...
pub fn pvt_cell_check_col(puzzle: &Matrix, possible_vals: &mut Vec<u8>, &col: &usize) {
    for r in 0..9 {
        possible_vals.retain(|&x| x != puzzle.vals[r][col]);
    }
}

pub fn pvt_cell_check_row(puzzle: &Matrix, possible_vals: &mut Vec<u8>, &row: &usize) {
    for c in 0..9 {
        possible_vals.retain(|&x| x != puzzle.vals[row][c]);
    }
}

pub fn build_pvt(puzzle: &Matrix) -> Vec<Vec<Option<Vec<u8>>>> {
    // PVT = Possible Values Table
